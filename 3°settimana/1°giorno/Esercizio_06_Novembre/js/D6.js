...
console.log(evenNumber([2, 3, 4, 5, 6, 7, 8, 9, 10, 12]));
/* ESERCIZIO 4 (forEach)
...
console.log(sum([3, 4, 5, 6, 7, 10]));

/* ESERCIZIO 5 (reduce)
  Scrivi una funzione per sommare i numeri contenuti in un array
*/const summ = (array) => {
  return array.reduce((tot, elemento) => tot += elemento);
}
console.log(summ([3, 3, 3, 3, 3, 5, 3]));
/* ESERCIZIO 6 (map)
  Scrivi una funzione che, dato un array di soli numeri e un numero n come parametri, ritorni un secondo array con tutti i valori del precedente incrementati di n
*/ const sumn = (array, n) => {
  return array.map((element) => element + n)
}
console.log(sumn([5, 6, 7, 8, 7], 10));
...
console.log(string(["EPICODE", "is", "great"]));
...
const oldestMovie = (array) =>{
  let risult ={ Year: 2100}
  array.forEach((movie)=>{
    let corrente= parseInt(movie.Year)
      if(corrente<risult.Year){
        risult=movie;
      }
    }
  )
...
const onlyInThisMillennium=(array)=>{
  return array.filter((el)=> parseInt(el.Year)>1999);
}
...
const sumYears=(array)=>{
  return array.reduce((tot,el)=> tot+= parseInt(el.Year),0);
}
console.log(sumYears(movies));
/* ESERCIZIO 14 (find)
  Scrivi una funzione per ottenere dall'array fornito uno specifico film (la funzione riceve un imdbID come parametro).
*/
const trova =(array,id)=>{
  return array.find((el)=> id=== el.imdbID);
}
...
const trovaIndice=(array,anno)=>{
  return array.findIndex((el)=>parseInt(el.Year)===anno);
}
